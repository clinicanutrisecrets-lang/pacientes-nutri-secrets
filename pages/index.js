  ...
  const Chatbot = () => (
    <>
      <div style={{position:'fixed',bottom:24,right:24,zIndex:1000,display:'flex',flexDirection:'column',alignItems:'flex-end',gap:8}}>
        {!chatOpen && (
          <div style={{background:'white',borderRadius:12,padding:'10px 16px',boxShadow:'0 4px 20px rgba(0,0,0,0.15)',border:'1px solid #d0e8e8',maxWidth:220,cursor:'pointer',animation:'fi 0.3s ease'}} onClick={()=>setChatOpen(true)}>
            <p style={{fontSize:'0.78rem',color:'#1a2e2e',fontWeight:500,lineHeight:1.4,margin:0}}>💬 Clique aqui para conversar com a Assistente de Nutrição Virtual</p>
          </div>
        )}
        <button onClick={()=>setChatOpen(o=>!o)} style={{width:58,height:58,borderRadius:'50%',background:'linear-gradient(135deg,#0ABDC0,#078b8e)',border:'none',cursor:'pointer',boxShadow:'0 4px 20px rgba(10,189,192,0.5)',display:'flex',alignItems:'center',justifyContent:'center',transition:'all 0.3s',fontSize:'1.5rem',flexShrink:0}}>
          {chatOpen?'✕':'💬'}
        </button>
      </div>
      {chatOpen && (
